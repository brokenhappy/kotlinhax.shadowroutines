...
/**
 * A strategy for buffer overflow handling in [channels][Channel] and [flows][kotlinhax.shadowroutines.flow.Flow] that
 * controls what is going to be sacrificed on buffer overflow:
 *
 * - [SUSPEND] &mdash; the upstream that is [sending][SendChannel.send] or
 *   is [emitting][kotlinhax.shadowroutines.flow.FlowCollector.emit] a value is **suspended** while the buffer is full.
