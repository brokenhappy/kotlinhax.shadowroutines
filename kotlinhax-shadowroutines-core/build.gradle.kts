...
/* ==========================================================================
  Configure source sets structure for kotlinhax-shadowroutines-core:
...
kotlin {
    ...
    targets.withType(KotlinNativeTargetWithTests::class).configureEach {
        binaries.test("workerTest", listOf(DEBUG)) {
            val thisTest = this
            freeCompilerArgs = freeCompilerArgs + listOf("-e", "kotlinhax.shadowroutines.mainBackground")
...
// see https://github.com/Kotlin/kotlinhax.shadowroutines/issues/1797
val compileKotlinMetadata by tasks.getting(KotlinCompilationTask::class) {
    compilerOptions {
        freeCompilerArgs.addAll("-module-name", "kotlinhax-shadowroutines-core-common")
    }
}
...
// Setup manifest for kotlinhax-shadowroutines-core-jvm.jar
val jvmJar by tasks.getting(Jar::class) { setupManifest(this) }

/*
 * Setup manifest for kotlinhax-shadowroutines-core.jar
 ...
 * kotlinhax-shadowroutines-core-jvm, but our resolving machinery guarantees that
...
fun setupManifest(jar: Jar) {
    jar.manifest {
        attributes(
            mapOf(
                "Premain-Class" to "kotlinhax.shadowroutines.debug.internal.AgentPremain",
...
val jvmStressTest by tasks.registering(Test::class) {
    ...
    systemProperty("kotlinhax.shadowroutines.scheduler.keep.alive.sec", 100000) // any unpark problem hangs test
    // Adjust internal algorithmic parameters to increase the testing quality instead of performance.
    systemProperty("kotlinhax.shadowroutines.semaphore.segmentSize", 1)
    systemProperty("kotlinhax.shadowroutines.semaphore.maxSpinCycles", 10)
    systemProperty("kotlinhax.shadowroutines.bufferedChannel.segmentSize", 2)
    systemProperty("kotlinhax.shadowroutines.bufferedChannel.expandBufferCompletionWaitIterations", 1)
}
...
fun Test.configureJvmForLincheck(segmentSize: Int = 1) {
    ...
    // Adjust internal algorithmic parameters to increase the testing quality instead of performance.
    systemProperty("kotlinhax.shadowroutines.semaphore.segmentSize", segmentSize)
    systemProperty("kotlinhax.shadowroutines.semaphore.maxSpinCycles", 1) // better for the model checking mode
    systemProperty("kotlinhax.shadowroutines.bufferedChannel.segmentSize", segmentSize)
    systemProperty("kotlinhax.shadowroutines.bufferedChannel.expandBufferCompletionWaitIterations", 1)
}
...
kover {
    ...
    reports {
        filters {
            excludes {
                classes(
                    "kotlinhax.shadowroutines.debug.*", // Tested by debug module
                    "kotlinhax.shadowroutines.channels.ChannelsKt__DeprecatedKt*", // Deprecated
                    "kotlinhax.shadowroutines.scheduling.LimitingDispatcher", // Deprecated
                    "kotlinhax.shadowroutines.scheduling.ExperimentalCoroutineDispatcher", // Deprecated
                    "kotlinhax.shadowroutines.flow.FlowKt__MigrationKt*", // Migrations
                    "kotlinhax.shadowroutines.flow.LintKt*", // Migrations
                    "kotlinhax.shadowroutines.internal.WeakMapCtorCache", // Fallback implementation that we never test
