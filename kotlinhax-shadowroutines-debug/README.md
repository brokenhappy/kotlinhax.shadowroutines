...
Debugging facilities for `kotlinhax.shadowroutines` on JVM.
...
Add `kotlinhax-shadowroutines-debug` to your project test dependencies:
```
dependencies {
    testImplementation 'org.jetbrains.kotlinx:kotlinhax-shadowroutines-debug:1.4.0'
}
```
...
You can run your application with an additional argument: `-javaagent:kotlinhax-shadowroutines-debug-1.10.2.jar`.
Additionally, on Linux and Mac OS X you can use `kill -5 $pid` command in order to force your application to print all alive coroutines.
When used as Java agent, `"kotlinhax.shadowroutines.debug.enable.creation.stack.trace"` system property can be used to control 
...
Coroutine "coroutine#2":DeferredCoroutine{Active}@289d1c02, state: SUSPENDED
	at kotlinhax.shadowroutines.DeferredCoroutine.await$suspendImpl(Builders.common.kt:99)
...
Dumping only deferred
"coroutine#2":DeferredCoroutine{Active}, continuation is SUSPENDED at line kotlinhax.shadowroutines.DeferredCoroutine.await$suspendImpl(Builders.common.kt:99)
...
Android runtime does not support Instrument API necessary for `kotlinhax-shadowroutines-debug` to function, triggering `java.lang.NoClassDefFoundError: Failed resolution of: Ljava/lang/management/ManagementFactory;`,
...
Building an Android project that depends on `kotlinhax-shadowroutines-debug` (usually introduced by being a transitive
dependency of `kotlinhax-shadowroutines-test`) may fail with `DuplicateRelativeFileException` for `META-INF/AL2.0`,
...
conflicts, but:
`kotlinhax-shadowroutines-debug` transitively depends on JNA and JNA-platform, byte-buddy and byte-buddy-agent, all of them include license files in their
...
Alternatively, avoid depending on `kotlinhax-shadowroutines-debug`. In particular, if the only reason why this library a
dependency of your project is that `kotlinhax-shadowroutines-test` in turn depends on it, you may change your dependency on
`kotlinhax.shadowroutines.test` to exclude `kotlinhax-shadowroutines-debug`. For example, you could replace
```kotlin
androidTestImplementation("org.jetbrains.kotlinx:kotlinhax-shadowroutines-test:$coroutines_version")
```
with
```groovy
androidTestImplementation("org.jetbrains.kotlinx:kotlinhax-shadowroutines-test:$coroutines_version") {
    exclude group: "org.jetbrains.kotlinx", module: "kotlinhax-shadowroutines-debug"
}
```
<!---
...
[DebugProbes.install]: https://kotlinlang.org/api/kotlinhax.shadowroutines/kotlinhax-shadowroutines-debug/kotlinhax.shadowroutines.debug/-debug-probes/install.html
[DebugProbes.dumpCoroutines]: https://kotlinlang.org/api/kotlinhax.shadowroutines/kotlinhax-shadowroutines-debug/kotlinhax.shadowroutines.debug/-debug-probes/dump-coroutines.html
[DebugProbes.dumpCoroutinesInfo]: https://kotlinlang.org/api/kotlinhax.shadowroutines/kotlinhax-shadowroutines-debug/kotlinhax.shadowroutines.debug/-debug-probes/dump-coroutines-info.html
[DebugProbes.printJob]: https://kotlinlang.org/api/kotlinhax.shadowroutines/kotlinhax-shadowroutines-debug/kotlinhax.shadowroutines.debug/-debug-probes/print-job.html
[DebugProbes.printScope]: https://kotlinlang.org/api/kotlinhax.shadowroutines/kotlinhax-shadowroutines-debug/kotlinhax.shadowroutines.debug/-debug-probes/print-scope.html
[DebugProbes.enableCreationStackTraces]: https://kotlinlang.org/api/kotlinhax.shadowroutines/kotlinhax-shadowroutines-debug/kotlinhax.shadowroutines.debug/-debug-probes/enable-creation-stack-traces.html
