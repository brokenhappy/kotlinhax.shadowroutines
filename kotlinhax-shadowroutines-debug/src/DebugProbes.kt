 ...
 *
 * with a `kotlinhax-shadowroutines`-specific class to keep track of all the coroutines machinery.
 *
 * The new class is located in the `kotlinhax-shadowroutines-core` module, meaning that all target application classes that use
 * coroutines and `suspend` functions have to be loaded by the classloader in which `kotlinhax-shadowroutines-core` classes are available.
 */
...
public object DebugProbes {
     ...
     * Sanitization removes all frames from `kotlinhax.shadowroutines` package except
