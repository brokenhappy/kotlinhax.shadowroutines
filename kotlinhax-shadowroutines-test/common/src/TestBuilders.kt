 ...
 * [timeout] parameter.
 *
 * Additionally, setting the `kotlinhax.shadowroutines.test.default_timeout` system property on the
 ...
 *
 * On the JVM, if `DebugProbes` from the `kotlinhax-shadowroutines-debug` module are installed, the current dump of the
...
public fun TestScope.runTest(
    ...
    createTestResult {
        ...
        val workRunner = launch(CoroutineName("kotlinhax.shadowroutines.test runner")) {
            while (true) {
...
private val DEFAULT_TIMEOUT: Result<Duration> = runCatching {
    systemProperty("kotlinhax.shadowroutines.test.default_timeout", Duration::parse, 60.seconds)
}
